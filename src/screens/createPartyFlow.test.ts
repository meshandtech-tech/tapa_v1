...
const base = {
  ...
  createCloudRoom: vi.fn(async () => ({ id: "room-1" }) as unknown | null),
...
describe("prepareRoom", () => {
  it("confirma a sala da nuvem somente depois de autenticar e criar no banco", async () => {
    ...
    const createCloudRoom = vi.fn(async () => ({ id: "room-1" }));

    await prepareRoom({ ...base, cloud: true, ensureSession, createCloudRoom });
